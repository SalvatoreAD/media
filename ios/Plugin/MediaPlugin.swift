...
public class MediaPlugin: CAPPlugin {
    ...
    func fetchResultAssetsToJs(_ call: CAPPluginCall) {
        ...
        fetchResult.enumerateObjects({ (asset, count: Int, stop: UnsafeMutablePointer<ObjCBool>) in
            ...
            self.imageManager.requestImage(for: asset, targetSize: PHImageManagerMaximumSize, contentMode: .aspectFill, options: requestOptions, resultHandler: { (fetchedImage, _) in
                ...
                var thumbnailImage = self.resizeImage(image: image, targetSize: thumbnailSize)
                if thumbnailImage != nil {
                    a["thumbnail_data"] = thumbnailImage?.jpegData(compressionQuality: CGFloat(thumbnailQuality) / 100.0)?.base64EncodedString()
                }
